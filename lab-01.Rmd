...
Non-applicable: there are non numerical values in the n_2021 variable, meaning mean and median cannot be calculated.
...
na.rm is a command to remove NA entries from the calculations.
...
The results have been skewed by a few very high visitor attraction at the top end.
...
```{r Ex_n}
visitors_with_nations %>%
  mutate(yr_pct = 100 * (n_2022 - n_2021) / n_2021) %>%
  filter(nation == "Scotland") %>%
  group_by(admission) %>%
  summarise(mean_pct_change = mean(yr_pct, na.rm = TRUE)) %>%
